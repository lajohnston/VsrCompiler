...
namespace VsrCompiler
{
    ...
    internal class LevelPack
    {
        ...
        public LevelPack(BinaryReader levelPackData, bool fullLemballVersion)
        {
            ...
            byte[] maxLevels;
            if (fullLemballVersion)
                maxLevels = new byte[]{ 25, 26, 29, 22, 12 };
            else
                maxLevels = new byte[] { 2, 1, 1, 1, 1 };

            // Initialise levelGroup list
            levelGroups = new LevelGroup[5];

            // Skip 2-byte null
            levelPackData.ReadInt16();

            // Compiler version
            uint compilerVersion = levelPackData.ReadUInt32();
